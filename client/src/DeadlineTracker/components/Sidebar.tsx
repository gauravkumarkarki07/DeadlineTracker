...
function Sidebar() {
    ...
    return (
        <nav className={`flex flex-col gap-12 min-h-screen bg-white px-6 py-4 text-black transition-all duration-300 ${isOpen ? 'w-64' : 'w-16'}`}>
            ...
            <section className='flex flex-col gap-14 text-sm font-normal'>
                ...
                <section className="flex flex-col gap-6 text-gray-600">
                {isOpen && <h2 className='font-semibold text-base text-black'>Settings</h2>}
                    <section className='flex items-center gap-2 cursor-pointer hover:text-accentPrimary' onClick={Logout}>
                        <LogOut size={18} />
